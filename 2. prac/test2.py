...
class CooSparseMatrix:
    ...
    def __add__(s, mat):
        if (mat.ra, mat.ca) != (s.ra, s.ca):
            raise TypeError

        res = CooSparseMatrix([], (s.ra, s.ca))
        res.im = dict(
            (rkey, dict((ckey, cvalue) for ckey, cvalue in rvalue.items())) for
            rkey, rvalue in s.im.items())
        for rkey, row in mat.im.items():
            if rkey not in res.im:
                s_l = res.im[rkey] = {}
                s_l.update(row)
            else:
                s_l = res.im[rkey]
